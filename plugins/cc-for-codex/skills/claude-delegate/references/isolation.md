...
Claude may leave the generated worktree locked even after a foreground session exits. A Git worktree lock prevents pruning; it does not make the registered checkout invalid. The bridge accepts only the expected Claude session lock, rejects foreign or initializing locks, refuses foreground success while the lock owner appears live, and blocks review while that owner or a matching Claude background agent appears active. It reports and preserves the lock without unlocking it. Exact-base verification fails closed if Claude commits inside the worktree; inspect its recovery location manually. Do not remove or unlock the worktree merely to make verification pass.
