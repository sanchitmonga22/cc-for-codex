...
| Print mode skips workspace trust | Fixed safe/no-customization read profile, explicit read-only tool list, and no interactive permission prompts; the read tools are not a filesystem sandbox and may access paths Claude permits outside the checkout |
