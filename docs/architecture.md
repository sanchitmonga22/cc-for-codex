...
Write mode requires Git, a self-contained full non-sparse/non-promisor checkout with no alternate/shared object store, no repository include/includeIf indirection, safe real-directory ancestry for `.claude/worktrees`, inspection of `.worktreeinclude`, and no selected content filter in the index, working attributes, or `HEAD`. It generates a unique Claude worktree name, requests `worktree.baseRef=head`, neutralizes Git checkout hooks/fsmonitor/global attributes, and exposes only read/file-edit built-ins. Claude creates `.claude/worktrees/<name>` on `worktree-<name>`; the bridge parses Git's NUL-delimited worktree metadata, rejects prunable entries, and verifies an existing canonical directory, regular `.git` marker, exact path, branch, and exact base commit. Claude may leave its own session lock after a foreground run; that lock prevents Git pruning but does not invalidate registration. The bridge accepts only the expected Claude session lock, rejects foreign or initializing locks, refuses foreground success while its owner appears live, and blocks review while the owner or a matching Claude background agent appears active. It reports and preserves the lock rather than unlocking or deleting the worktree. This concurrency check uses host PID and Claude lifecycle signals, not an atomic filesystem snapshot or OS isolation. If Claude commits in the worktree, exact-base verification fails closed for manual inspection. The bridge never merges or pushes.
